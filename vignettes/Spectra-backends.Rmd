...
function that takes (among other possibilities) a `data.frame` with the full MS
data as input. Each row in that `data.frame` is expected to contain data from
one spectrum. We therefore need to add the *m/z* and intensity values within a
`list` to our data frame.
...
So, we've now got a `Spectra` object representing a single MS spectrum - but, as
the name of the class implies, it is actually designed to represent data from
...
A `Spectra` *behaves* thus similar to a `data.frame` with rows being individual
...
e.g. `matrix` with columns being the peaks variables and rows the values for
each peak. The number of rows of such peak variable arrays depends on the number
of mass peaks of each spectrum. Below we extract the peaks data of our `Spectra`
and subset that to data from the second spectrum.
...
Or to create such a spectrum plot *manually*:
...
these formats. Below we create a `Spectra` object for such experimental data
from two mzML files that are provided in Bioconductor's `r Biocpkg("msdata")`
package.
...
object. In contrast to the first example we did however in this case not provide
a `data.frame` with the MS data as input parameter to the `Spectra` call, but a
`character` vector with the file names from which to import the data. Also, we
specified in addition `source = MsBackendMzR()`. We thus already anticipate the
concept of backends. With this parameter we tell `Spectra` to use the
`MsBackendMzR` backend to import and represent the MS data from the two mzML
files.

Like in our first example, we can subset the object or access any of its spectra
variables. Note that for this `Spectra` object we have many more spectra
variables available (depending on what is provided by the raw data files).
...
accessed in the same way, they actually use different *backends* and thus data representations:
...
says, keeps all MS data in memory (within the backend object). In contrast, the
...
The backend that is able to import data from files in a certain format needs to
be specified in the `Spectra` constructor call with the `source` parameter:
...
performance or for low memory demand.
...
port or password. Which of these parameters are required depends on the SQL
...
backends. We compare the time needed to extract the retention times from the 3
`Spectra` object using the `microbenchmark` function. Also here we disable any
potential parallel processing using `register(SerialParam())`.
...
`MsBackendOfflineSql` from the SQL database (which for this data is faster).
...
and processing the full data as a whole. The performance of the *offline*
backends `MsBackendMzR` and `MsBackendSql`/`MsBackendOfflineSql` depend also on
the I/O capabilities of the hard disks containing the data files as well as, for
the latter, the configuration of the SQL database system used.
...
to allow any data changes to be written back to these databases.

Any type of analysis requires however some sort of data manipulation, thus, we
need for such backends some sort of mechanism to *cache* changes to the data.
...
from it and does not need to implement its own. Examples of backends extending
