...
We can retrieve the respective release and access its spectra data with the code
below. Note that *AnnotationHub* will cache the downloaded database locally and
any subsequent call will not download the database again.
...
The `MsBackendSql` allows now to access and use the MS data stored in such a
database. We simply provide the connection to the database as a parameter to the
`Spectra` function and use the `MsBackendSql` as *backend*.

```{r}
sps_db <- Spectra(scon, source = MsBackendSql())
sps_db
```
...
`MsBackendMzR`. Below we compare the performance of the `MsBackendSql` on the
...
Thus, the `MsBackendSql` has similar performance than having all the data in
