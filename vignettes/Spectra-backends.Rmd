...
function accepts (among other possibilities) a `data.frame` with the full MS
data as input. Each row in that `data.frame` is expected to contain data from
...
The full peaks data can be extracted with the `peaksData` function, that returns
a `list` of two-dimensional arrays with the values of the peaks variables. Each
list element represents the peaks data from one spectrum, which is stored in a
e.g. `matrix` with columns being the peaks variables and rows the respective
values for each peak. The number of rows of such peak variable arrays depends on
the number of mass peaks of each spectrum. This number can be extracted either
using `lengths`:
...
We next extract the peaks data of our `Spectra` and subset that to data from the
second spectrum.
...
Such plots can obviously also be created *manually* from the individual peaks
data:
...
these formats. Also *Spectra* allows to represent data from such raw data files:
below we create a `Spectra` object for such experimental data from two mzML
...
`source = MsBackendMzR()`. We thus already anticipate the concept of
backends. With the `source` parameter we configured `Spectra` to use the
`MsBackendMzR` backend to import and represent the MS data from the two mzML
files.
...
accessed in the same way, they actually use different *backends* and thus data
representations:
...
data (variables) into memory while it imports the peaks data only *on-demand*
...
To import data from files in a certain format, the respective backend able to
handle such data needs to be specified with the parameter `source` in the
...
requirements are particularly small and is thus ideal to represent even very
large MS experiments.

We next evaluate the performance to extract spectra variables. We compare the
time needed to extract the retention times from the 3 `Spectra` object using the
`microbenchmark` function. Also here we disable any potential parallel
processing using `register(SerialParam())`.
...
`Spectra` with the `MsBackendOfflineSql`.
...
`MsBackendOfflineSql` from the SQL database (which for the present data is
more efficient).
...
while the `MsBackendMemory` needs to subset the full data, and the
...
as well as, for the latter, the configuration of the SQL database system used.
...
supposed to allow any data changes to be written back to these databases.

Data manipulations are however an integrative part of any data analysis and we
thus need for such backends some mechanism that allows changes to data without
propagating these to the original data resource. Some backend classes use
dediacted *caching* mechanisms to enable adding, changing, or deleting spectra
variables. To illustrate how this is implemented for our 3 backend classes we
below add a new spectra variable `"new_variable"` to each of our 3 `Spectra`
objects.
...
peaks variables!) within a `DataFrame`. Adding a new spectra variable thus also
added a new column to this `DataFrame`. *Note*: the use of a `DataFrame` instead
...
cached within a dedicated `data.frame` inside the object.
...
provides the caching mechanism for spectra variables of the
`MsBackendOfflineSql` backend above. Thus, any `MsBackend` implementation
extending this base class automatically inherit this mechanism from it and does
not need to implement its own. Examples of backends extending `MsBackendCached`
are, among others, `MsBackendSql`, `MsBackendOfflineSql`, and
`MsBackendMassbankSql`.

Analysis of MS data requires, in addition to changing or adding spectra
variables also the possibility to manipulate peaks data (i.e. the
...
(*m/z* and intensity values of the individual peaks of MS spectra) however
represent much larger data volumes. Thus, in-memory caching of changes to peaks
data is not generally possible for all types of MS data.
...
This lazy processing mechanism is implemented in the `Spectra` object and is
thus independent of the backend used to represent the data. Thus, it allows to
apply also data manipulations to MS data from data resources that would be
inherently *read-only* (such as e.g. reference libraries of fragment spectra).
