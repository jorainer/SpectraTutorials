...
is available [here](https://hub.docker.com/r/jorainer/spectra_tutorials). This
docker image (given that docker is installed on the computer) can be installed
with `docker pull jorainer/spectra_tutorials:latest`.
...
- Optionally, to run also the code to import the MS2 spectra from HMDB the *All
  Spectra Files (XML)* from the [hmdb downloads page](https://hmdb.ca/downloads)
  have to be downloaded. The contents of the *hmdb_all_spectra.zip* archive
  should then be unzipped into the folder *data/hmdb_all_spectra*.
- Start docker using
  ```
  docker run \
      -e PASSWORD=bioc \
      -p 8787:8787 \
      jorainer/spectra_tutorials:devel
  ```
...
- Open this R-markdown file
  ...
  RStudio server version in the web browser.

Alternatively to the docker-based usage it is also possible to install all
required R packages using the code below (using an [R](https://r-project.org)
...
To use the examples with the `MsBackendMassBank` a local *MassBank* installation
is required, which requires access to a MySQL or MariaDB server with the
possibility to create new databases. A MySQL database dump of the `MassBank`
database can be downloaded from [the official github
...
## Access to other public spectral libraries

There are also other (public) spectral libraries for small molecules available
such as [MassBank](https://massbank.eu/MassBank/),
[MoNa](https://mona.fiehnlab.ucdavis.edu/) (MassBank of North America) or
[GNPS](https://gnps.ucsd.edu/ProteoSAFe/static/gnps-splash.jsp). Ultimatively
the goal is to provide `MsBackend` interfaces for all of these to allow a direct
and unified access to these resources from within R. For the MassBank database
backends are already available in the
[MsBackendMassbank](https://github.com/michaelwitting/MsBackendMassbank)
package. This package provides the `MsBackendMassbank` backend for import/export
of [MassBank files](https://github.com/MassBank/MassBank-data) as well as the
`MsBackendMassbankSql` that directly interfaces the MassBank MySQL database.

Below we load the `MsBackendMassbank` library and connect to a local
installation of the MassBank MySQL database (release *2020.10* which is provided
