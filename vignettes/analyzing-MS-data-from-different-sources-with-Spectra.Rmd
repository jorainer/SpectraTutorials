...
vignette: >
  ...
  %\VignetteDepends{Spectra,mzR,RMariaDB,pheatmap,BiocStyle,CompoundDb}
...
database can be loaded instead using:
...
`filterPrecursorMz` used below, we could also use the `containsMz` function to
screen for spectra containing an actual peak matching the precursor m/z.
...
MoNA, MassBank or HMDB. A `CompDb` database containing all compound annotations
along with all (experimental) MS2 spectra is provided as a data release in the
[MetaboAnnotationTutorials](https://jorainer.github.io/MetaboAnnotationTutorials/)
...
- Caching data operations and applying them on-the-fly allow data manipulations
