...
vignette: >
  ...
  %\VignetteDepends{Spectra,mzR,RMariaDB,pheatmap,BiocStyle,CompoundDb,curl,MsBackendSql,MsBackendMassbank,RSQLite,AnnotationHub}
...
- The present version of the tutorial bases on package versions available
  through the **Bioconductor release 3.19**.
...
*variables* (such as `"rtime"`, i.e. the retention time). The
`spectraVariables()` function lists all available variables within such a
`Spectra` object.
...
`intensity()` functions or also with the `peaksData()` function that returns a
list of numeric matrices with the m/z and intensity values. By using an
...
object. Note that, for scaling of peak intensities, there would also be a
dedicated function, `scalePeaks()`, available.
...
matching. By default, `compareSpectra()` uses the (normalized) dot-product to
calculate the similarity between the spectra, but a variety of different
algorithms can be used instead (see the [*Spectra
vignette*](https://rformassspectrometry.github.io/Spectra/articles/Spectra.html#comparing-spectra)
...
*2023.11* which is provided within the docker image of this tutorial).
...
with a `MsBackendMassbankSql` backend providing it with the database connection.
...
not be run within *docker*, the MassBank data can also be retrieved from
Bioconductor's `r Biocpkg("AnnotationHub")`. In that case `MsBackendCompDb`
backend (defined in the `r Biocpkg("CompoundDb")`) is used instead, but both
backends retrieve their data from a SQL database and have the same
properties. Below we list MassBank releases/versions available in Bioconductor's
*AnnotationHub*.
...
We can retrieve the respective release (*2021.03* in this case) and access its
spectra data with the code below. Note that *AnnotationHub* will cache the
downloaded database locally and any subsequent call will not download the
database again.
...
data is requested (i.e. whenever `mz()` or `intensity()` is called on it). As a
side effect, this also allows to *undo* certain data operations by simply
calling `reset()` on the `Spectra` object.
...
tested MassBank spectrum (columns) against each experimental spectrum (rows).
...
showing the best matches between experimental spectra (upper panel) and fragment
spectra from MassBank (lower panel). Matching peaks are highlighted with a
...
plotSpectraMirror(sps[best_match[1]], mbank_sub[best_match[2]],
...
plotSpectraMirror(sps[3], mbank_sub[1],
                  ppm = 20, labels = label_fun, labelPos = 2,
                  labelOffset = 0.2, labelSrt = -30)
grid()
```
...
each spectrum. Thus, we below use the `spectraVariables()` function to list all
of the variables that are available in our MassBank `Spectra` object.
...
name. Note that the database provided by *AnnotationHub* does contain compound
names in a column/spectra variable `"name"` instead of `"compound_name"`, thus,
if we used the option to download the MassBank data from *AnnotationHub* above,
we would have to use `mbank_best_match$name` instead of
`mbank_best_match$compound_name` below.

```{r massbank-one-spectrum}
mbank_best_match <- mbank_sub[best_match[2]]
...
and we hence need to use the `containsMz()` function to find spectra containing
a peak with an m/z matching the m/z of our ion of interest. In addition, We need
...
experiments in a central place. The `MsBackendSql` backend allows for example to
...
createMsBackendSqlDatabase(scon, fls)
dbDisconnect(scon)
```

The `MsBackendSql` or `MsBackendOfflineSql` backends allow now to access and use
the MS data stored in such a database. The `MsBackendOfflineSql` does not keep
an active connection to the database but reconnects to the database when
needed. While having a slightly lower performance, this allows the object to be
*serialized*, e.g. saved to and reloaded from disk using the `save()` and
`load()` functions. To access the data within the SQL database We simply the
name to the database as a parameter to the `Spectra()` function and use the
`MsBackendOfflineSql` as *backend*. For the `MsBackendOfflineSql` we need to
specify the SQL *driver*, in our case, since we are using a SQLite database, it
will be `SQLite()` as well as the name of the database (which for SQLite is the
file name of the database).

```{r}
sps_db <- Spectra(db_file, drv = SQLite(),
...
`MsBackendMzR`. Below we compare the performance of the `MsBackendOfflineSql` on
the present small data set to a `MsBackendDataFrame` (data in-memory) and a
`MsBackendMzR` (data retrieved from original file). Note that in more recent
versions of the *Spectra* package a higher-performance in-memory backend would
be available (`MsBackendMemory`).
...
Thus, the `MsBackendOfflineSql` has similar performance than having all the data
in memory. The performance is even better if smaller subsets of the data are
loaded.
...
- [`MsQuality`](https://github.com/tnaake/MsQuality) (Thomas Naake): QC measures
  for MS data [@naake_msquality_2023].
