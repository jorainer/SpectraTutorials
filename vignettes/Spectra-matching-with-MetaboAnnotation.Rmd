...
Italy; johannes.rainer@eurac.edu], Michael Witting^[Research Unit Analytical
BioGeoChemistry, Helmholtz Zentrum München and Chair of Analytical Food Chemistry, TUM School or Life Sciences, Technical University of Munich, Germany]"
output:
...
package that allows to restrict similarity calculations only between query and
target spectra that have a similar precursor m/z. We thus create below a
`CompareSpectraParam` object with parameter `requirePrecursor = TRUE` (to
restrict similarity calculations only to query and target spectra with a similar
precursor m/z) and `ppm = 10` (m/z difference between the query and target
precursor has to be within 10 ppm). Parameter `THRESHFUN` enables to define a
*threshold function* that defines which spectra are considered matching. With
the function used below only MS2 spectra with a similarity (calculated with the
default *dotproduct* function) larger or equal to 0.8 are considered matching.
...
As a result we got a `MatchedSpectra` object that contains the query and target
...
`r length(query(mtch))` spectra) could be matched. In part this is because a big
proportion of the MassBank spectra have a missing precursor m/z and with
`requirePrecursor = TRUE` they would not be considered in the similarity
...
We can access spectra variables using `$` and the variable name, or by passing
the variable name(s) to the `spectraData` function. Below we thus extract the
retention time the precursor m/z or the query spectrum, the precursor m/z of the
target spectrum as well as the similarity score from the object.
...
spectra that could not be matched with a target spectrum. If we're only
interested in query spectra for which a match was found, we could simply subset
the `MatchedSpectra` like below.
...
results for the 4th query spectrum. We now extract now the matching information
after subsetting
...
and target spectra. Pre-filtering the spectra prior the actual spectra
...
MS1 data requires a chromatographic peak detection step which is most frequently
performed using the functionality from the `r BiocStyle::Biocpkg("xcms")`
package. Below we load thus the `xcms` package and read the data using the
`readMSData` function.
...
package for details about chromatographic peak detection related settings).
...
`chromPeakSpectra` function. By setting `return.type = "Spectra"` we ensure the
data to be returned in the newer `Spectra` format hence enabling the simplified
spectra matching with the functionality presented here.
