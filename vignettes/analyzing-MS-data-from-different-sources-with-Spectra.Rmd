...
For subsetting `Spectra` object we can use either `[` or one of the many
available `filter*` functions (that are usually more efficient than `[`
subsetting). Below we subset the spectra to some arbitrary spectra.
...
m/z of the *[M+H]+* ion of the metabolite cystine (one of the standards added to
the sample mix measured in the present experimental data) accepting a difference
