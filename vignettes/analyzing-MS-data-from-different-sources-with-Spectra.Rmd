...
The pairwise spectra similarities are represented with the heatmap below. We
directly export the heatmap to a pdf file as R (or RStudio) in the docker
occasionally crashes displaying the interactive graphic.

```{r ms2-heatmap, fig.width = 7, fig.height = 7}
library(pheatmap)
hm <- pheatmap(cormat, cutree_rows = 3, file = "ms2-heatmap.pdf")
```
